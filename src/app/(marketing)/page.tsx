    ...
    <section className="relative overflow-hidden">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 pt-14 sm:pt-20 pb-10 sm:pb-16 text-center">
        ...
        <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link href="/signup"><Button size="lg" className="h-12 px-6 text-sm sm:text-base bg-primary hover:bg-primary/90 w-full sm:w-auto rounded-full">Build your agent free <ArrowRight className="ml-2 h-4 w-4" /></Button></Link>
          <Link href="/contact"><Button size="lg" variant="ghost" className="h-12 px-6 text-sm sm:text-base w-full sm:w-auto rounded-full">Book a demo</Button></Link>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-2.5 sm:space-y-3 text-sm scroll-smooth">
        ...
        {typing && (
          <div className="animate-fade-in">
            <div className="max-w-[85%] rounded-3xl bg-white border border-border">
              <TypingIndicator />
            </div>
          </div>
        )}
      ...
      {VOICE_TRANSCRIPT_LINES.slice(0, transcriptIndex + 1).map((line, i) => {
        ...
        return (
          <div key={i} className={`flex ${isAgent ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[90%] rounded-3xl px-3 py-2 text-xs leading-relaxed ${isAgent ? "bg-primary/10 text-foreground" : "bg-[color:var(--surface-2)] text-foreground"}`}>
...
function HeroDemo() {
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 pb-12 sm:pb-16 md:pb-24">
      <div className="relative rounded-3xl border border-border bg-white shadow-[0_20px_60px_-25px_rgba(0,0,0,0.15)] overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-0 min-h-[300px] sm:min-h-[360px] md:min-h-[420px]">
          ...
          <div className="hidden md:flex flex-col p-6 bg-white">
            ...
            <div className="mb-4 rounded-3xl border border-border bg-[color:var(--surface-2)] p-3">
              <VoiceWaveform />
            ...
            <div className="flex-1 rounded-3xl border border-border bg-[color:var(--surface-2)] p-3 overflow-hidden">
...
function Bubble({ side, children }: { side: "in" | "out"; children: React.ReactNode }) {
  return (
    <div className={`max-w-[85%] rounded-3xl px-4 py-2.5 leading-snug ${side === "in" ? "bg-white border border-border" : "ml-auto bg-[#dcf8c6]"}`}>
      {children}
    </div>
  );
}
        ...
        {highlights.map((h, i) => {
          ...
          return (
            <div key={h.title} className="animate-fade-up">
              <div className="mb-4 grid h-10 w-10 place-items-center rounded-2xl bg-accent-soft text-primary"><I size={20} /></div>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-10 sm:mt-12 md:mt-16 grid md:grid-cols-2 gap-8 sm:gap-12">
          ...
          <div className="hidden md:block">
            <div className="sticky top-24 rounded-3xl border border-border bg-white p-6 min-h-[420px] shadow-sm">
              ...
              <div className="mt-6 rounded-3xl border border-border bg-[color:var(--surface-2)] p-4 min-h-[280px] animate-fade-up" key={active}>
                {demos[active]}
              </div>
            </div>
          </div>
  ...
  return (
    <section className="bg-background py-14 sm:py-20 md:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="rounded-3xl border border-border bg-[color:var(--surface-2)] p-4 sm:p-6 min-h-[320px] sm:min-h-[380px] md:min-h-[440px]">
...
function DemoNumber() {
  return <div className="space-y-3"><div className="flex items-center gap-3 rounded-3xl border border-border bg-card p-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-whatsapp text-primary-foreground">W</span><div><div className="font-semibold">Main WhatsApp connected</div><div className="text-xs text-muted-foreground">Verification AL-4821 accepted</div></div></div><div className="rounded-3xl bg-card p-3 text-xs font-mono text-muted-foreground">webhook: live · routing: agent_first · fallback: owner</div></div>;
}

function DemoCatalog() {
  return <div className="space-y-2">{[["Balayage", "$45", "120m"], ["Blowout", "$12", "30m"], ["Bridal package", "$85", "180m"]].map((row) => <div key={row[0]} className="grid grid-cols-3 rounded-3xl border border-border bg-card p-3 text-sm"><span className="font-semibold">{row[0]}</span><span className="font-mono text-right">{row[1]}</span><span className="font-mono text-right text-muted-foreground">{row[2]}</span></div>)}</div>;
}

function DemoConversation() {
  return <div className="space-y-3 text-sm"><Bubble side="in">Can two people come Saturday after 3?</Bubble><Bubble side="out">Yes — Ana has 3:30 open. I can hold it for both of you.</Bubble><div className="rounded-3xl border border-primary/20 bg-accent-soft p-3 text-xs text-primary">Action queued: book_appointment · confidence 94%</div></div>;
}

function DemoEscalation() {
  return <div className="space-y-3"><div className="rounded-3xl border border-warning/30 bg-warning/10 p-3"><div className="text-sm font-semibold text-warning">Human review needed</div><div className="text-xs text-muted-foreground">Refund request over policy threshold</div></div><div className="rounded-3xl border border-border bg-card p-3 text-xs">Summary sent with customer history, order ID, and suggested response.</div></div>;
}

function DemoAnalytics() {
  return <div className="space-y-3"><div className="grid grid-cols-3 gap-2">{[["Closed", "82%"], ["Revenue", "$4.8k"], ["Saved", "31h"]].map(([a, b]) => <div key={a} className="rounded-3xl bg-card p-3"><div className="text-[10px] uppercase tracking-widest text-muted-foreground">{a}</div><div className="font-mono font-bold">{b}</div></div>)}</div><div className="flex h-24 items-end gap-2 rounded-3xl bg-card p-3">{[30, 45, 62, 50, 78, 88, 70].map((h, i) => <div key={i} className="flex-1 rounded-t-full bg-primary" style={{ height: `${h}%` }} />)}</div></div>;
}
...
function FeatureDemo({ title, index }: { title: string; index: number }) {
  ...
  return <div className="rounded-3xl border border-border bg-[color:var(--surface-2)] p-4 sm:p-6 min-h-[200px] sm:min-h-[260px] text-sm"><div className="rounded-3xl bg-white border border-border p-3 sm:p-4 shadow-sm"><div className="mb-3 flex items-center justify-between"><span className="text-xs font-mono text-muted-foreground">{title}</span><span className="rounded-full bg-success/10 px-2 py-0.5 text-[10px] font-semibold text-success">LIVE</span></div>{index === 0 ? rows.map(([a, b, c]) => <div key={a} className="mb-2 grid grid-cols-[80px_1fr_auto] sm:grid-cols-[90px_1fr_auto] gap-1.5 sm:gap-2 rounded-2xl bg-[color:var(--surface-2)] p-2 text-xs last:mb-0"><span>{a}</span><span className="font-medium">{b}</span><span className="text-muted-foreground">{c}</span></div>) : index === 1 ? <div className="grid grid-cols-4 gap-1.5 sm:gap-2">{["09", "10", "11", "12", "13", "14", "15", "16"].map((h, n) => <div key={h} className={`rounded-2xl p-1.5 sm:p-2 text-center text-[10px] sm:text-xs ${n === 6 ? "bg-accent-soft text-primary" : "bg-[color:var(--surface-2)]"}`}>{h}:00</div>)}</div> : <div className="space-y-2">{["Model A answered in 1.2s", "Escalation rule matched", "Action safely completed"].map((x) => <div key={x} className="rounded-2xl bg-[color:var(--surface-2)] p-2 text-xs">{x}</div>)}</div>}</div></div>;
}

function PlatformPreview({ active, label }: { active: string; label: string }) {
  return <div className="rounded-3xl bg-white border border-border min-h-[260px] sm:min-h-[320px] md:min-h-[380px] p-4 sm:p-6 shadow-sm"><div className="text-xs font-mono text-muted-foreground mb-3">{label} — live preview</div><div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4"><div className="sm:col-span-2 rounded-3xl border border-border bg-[color:var(--surface-2)] p-3 sm:p-4"><div className="mb-3 sm:mb-4 flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-success animate-pulse-dot" /><span className="text-xs sm:text-sm font-semibold">{active === "analytics" ? "Revenue influenced" : active === "inbox" ? "Customer thread" : active === "catalog" ? "Service catalog" : active === "actions" ? "Automation timeline" : "Agent playground"}</span></div>{active === "analytics" ? <DemoAnalytics /> : active === "catalog" ? <DemoCatalog /> : active === "actions" ? <DemoEscalation /> : <DemoConversation />}</div><div className="space-y-2 sm:space-y-3">{["Confidence 94%", "Tool call ready", "Owner notified"].map((x) => <div key={x} className="rounded-3xl bg-[color:var(--surface-2)] p-3 sm:p-4 text-xs font-semibold">{x}</div>)}</div></div></div>;
}
      ...
      <div className="relative">
        <div className="flex gap-3 sm:gap-4 animate-marquee w-max">
          {[...testimonials, ...testimonials].map((t, i) => (
            <div key={i} className="w-[280px] sm:w-[340px] md:w-[380px] rounded-3xl border border-border bg-white p-4 sm:p-6 shadow-sm">
  ...
  return (
    <section className="bg-background py-12 sm:py-16 md:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
        {items.map((it) => (
          <div key={it.title} className="rounded-3xl border border-border bg-white p-4 sm:p-5 md:p-6">
...
function BigCTA() {
  return (
    <section className="px-4 sm:px-6 py-10 sm:py-16">
      <div className="relative overflow-hidden rounded-[2rem] bg-[#0a0a0b] text-[#f2f2f3] mx-auto max-w-6xl">
        ...
        <div className="relative px-5 sm:px-8 py-14 sm:py-20 md:py-24 text-center">
          ...
          <div className="mt-6 sm:mt-8">
            <Link href="/signup"><Button size="lg" className="h-12 px-6 text-sm sm:text-base bg-white text-black hover:bg-white/90 w-full sm:w-auto rounded-full">Build your agent free</Button></Link>
          </div>
