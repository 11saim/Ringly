...
export default function BookingsPage() {
  ...
  const byStaff = useMemo(
    () =>
      staff.map((name) => ({
        name,
        count: list.filter((b) => b.staff === name).length,
      })),
    [list],
  );
  ...
  const confirmWaitlist = (id: string) => {
    setList((items) =>
      items.map((b) => (b.id === id ? { ...b, status: "confirmed" } : b)),
    );
  ...
  return (
    <AppShell
      title="Bookings"
      subtitle={`${list.length} upcoming · ${confirmed} confirmed · ${pending.length} waitlisted`}
      actions={
        <Button
          type="button"
          size="sm"
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-xl border border-border bg-card p-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="inline-flex flex-wrap gap-1 rounded-md border border-border bg-background p-0.5">
              {TABS.map((item) => {
        ...
        {/* ── Content + optional waitlist sidebar ── */}
                  ...
                  <div key={booking.id} className="rounded-lg border border-border p-3 text-sm">
                    ...
                    <div className="mt-3 flex gap-2">
                      <Button
                        ...
                        onClick={() => confirmWaitlist(booking.id)}
                      >
                        Confirm
                      </Button>
                      <Button
                        ...
                        className="h-7 text-xs"
                        onClick={() =>
                          setList((items) => items.filter((b) => b.id !== booking.id))
                        }
                      >
                        Decline
                      </Button>
      ...
      <Dialog open={newOpen} onOpenChange={setNewOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a booking and place it instantly on every view.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!form.customer.trim() || !form.service.trim()}
            >
              Create booking
            </Button>
          </DialogFooter>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card">
      <div
        ...
        style={{
          gridTemplateColumns: `64px repeat(${days.length}, minmax(108px, 1fr))`,
        }}
      >
        {/* Header row */}
        ...
        {days.map((day) => (
          <div
            key={day}
            className="border-b border-l border-border bg-muted/40 px-3 py-3 text-xs font-semibold text-muted-foreground"
          >
            {day}
          </div>
        ))}
        ...
        {hours.map((hour) => (
          <div key={hour} className="contents">
            <div className="border-t border-border px-2 py-4 text-xs font-mono text-muted-foreground">
              {hour}
            </div>
            {days.map((day) => {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-3">
        {staffLoad.map((person) => (
          <Metric
            key={person.name}
            icon={<UsersRound />}
            label={person.name}
            value={`${person.count} jobs`}
          />
        ))}
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <div
          ...
          style={{
            gridTemplateColumns: `64px repeat(${staff.length}, minmax(150px, 1fr))`,
          }}
        >
          <div className="border-b border-border bg-muted/40" />
          {staff.map((person) => (
            <div
              key={person}
              className="border-b border-l border-border bg-muted/40 px-3 py-3 text-xs font-semibold"
            >
              {person}
            </div>
          ))}
          {hours.map((hour) => (
            <div key={hour} className="contents">
              ...
              {staff.map((person) => (
                <Slot
                  key={`${person}-${hour}`}
                  booking={list.find((b) => b.staff === person && b.time === hour)}
                />
              ))}
            </div>
          ))}
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            ...
            <Badge variant="secondary">
              {list.filter((b) => b.status === lane.toLowerCase()).length}
            </Badge>
          </div>
          <div className="space-y-2">
            {list
              .filter((b) => b.status === lane.toLowerCase())
              .map((booking) => (
                <BookingCard key={`${lane}-${booking.id}`} booking={booking} />
              ))}
          </div>
        </div>
      ))}
        ...
        <tbody>
          {list.map((booking) => (
            <tr key={booking.id} className="border-t border-border">
              <td className="px-4 py-3 font-semibold">{booking.customer}</td>
...
function Slot({ booking }: { booking?: Booking }) {
  return (
    <div className="min-h-[58px] border-l border-t border-border p-1.5">
      {booking && <BookingCard booking={booking} compact />}
    </div>
  );
}
...
function BookingCard({
  ...
  return (
    <div
      className={`rounded-lg border border-primary/20 bg-accent-soft p-2 text-primary shadow-sm ${
        compact ? "text-[11px]" : "text-sm"
      }`}
    >
      <div className="truncate font-semibold">{booking.customer}</div>
      <div className="truncate opacity-75">{booking.service}</div>
      {!compact && (
